...
class _BuscarWebInput(BaseModel):
    consulta: str = Field(
        ...
        "E.g.: 'latest EU AI Act enforcement dates', 'population of Chile 2025'."
    )
...
def _build_executable(name: str, ctx: RunContext) -> StructuredTool:
    ...
    if name == "buscar_en_web":
        ...
        return StructuredTool(
            name="buscar_en_web",
            description=(
                "Searches the public web (DuckDuckGo) for up-to-date or external "
                "information that is NOT in the project's documents: current facts, "
                "figures, news, definitions, references. Returns a list of results "
                "with title, URL and snippet. Use it to complement — not replace — "
                "the project documents; for data that lives in the uploaded files use "
                "buscar_en_documentos instead. Cite the source URLs when you rely on a result."
            ),
            func=_search_web,
